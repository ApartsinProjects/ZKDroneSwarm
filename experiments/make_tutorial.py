...
A(plain("A framing point first, because it changes how to read everything below. The ZK-MRTA setting "
        ...
        "full-information upper bounds. Being honest about who built what: our invented methods are ONE "
        "family, SwarmCF (the core plus Bayesian, choice, de-confliction, unified and batch variants, "
        "written 'SwarmCF-variant'), and PTF (SwarmCF-batch) is ours too, a batch-flavored hybrid we built "
        "from standard parts; MFSGD, ESTR, BPMF, SoftImpute, KNNCF and CLUB are named prior-work estimators "
        "we ADAPT to the setting (the algorithm is theirs, the decentralized broadcast-only application is "
        "part of our framework); UCB and tabular are the structure-free paradigm we beat; and three "
        "full-information upper bounds (not competitors) sit on top: the Oracle (knows the true reward), "
        "CentralClean (a centralized low-rank matcher that sees everything with no noise and no masking and "
        "assigns optimally, the best a centralized low-rank system can do without the true factors), and "
        "the noisy comms-full CTDE matcher. And one crucial implementation fact: our evaluation builds ONE "
...
A("<div class='step'><h3>Newest theory (a self-audit pass): scope corrected, precision fixed, a regret reason, and the keystone now bounded</h3>"
  ...
  "<p><b>The keystone, now bounded (was the central open problem).</b> Our headline 'a few samples "
  "complete the whole row' leans on a step, recovering the shared structure $U$ from the MASKED "
  "broadcast, that we previously only CITED from centralized completion theory. We now pin it to an "
  "explicit per-drone condition: drone $i$ recovers a target's factor $u_j$, and so predicts the unseen "
  "pair, EXACTLY when its own factor $p_i$ is spanned by the factors of the visible teammates that "
  "engaged $j$ (an anchor block to fix the frame, plus enough spanning engagers per target); it is "
  "provably stuck at the prior floor below that, the per-target version of the tabular floor; under noise "
  "the error is bounded by the fold-in bound with the coverage degree setting the noise term; and under "
  "ordinary exploration the condition becomes true on its own after about $nd/(\\rho m)\\cdot\\log n$ "
  "rounds, sooner as the swarm grows. An oracle-reconstruction check on the swarm's real coverage "
  "confirms the threshold exactly: pairs meeting the span condition reconstruct to error 0.000, the rest "
  "sit at the floor. The single remaining open piece is the finite-time coverage rate when drones "
  "exploit (an adaptive policy can starve low-reward targets of coverage). A much narrower gap than "
  "'is decentralized masked recovery even possible'. (Full statements, proofs, and the validation: "
  "<code>docs/THEORY_FORMAL.md</code>, <code>docs/P15_VALIDATION.md</code>.)</p></div>")
