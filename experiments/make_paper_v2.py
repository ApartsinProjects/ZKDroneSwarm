...
A("<div class='box'><b>Capstone: the menu composes into ONE method (UnifiedCF).</b> The refinements "
  ...
  "contests, Theorem 7); exploration anneals when the agent is losing AND when the offer is abundant "
...
A("<p><b>Why a swarm: collaboration value and positive scaling (Fig. 5).</b> Two experiments isolate "
  ...
  "skill to RewardCF (and $+0.51$ to PTF) but only $-0.00$ / $+0.01$ to the structure-free "
  "UCBIndep / Tabular, which have no model linking tasks and so cannot use the broadcast at all. This is "
  "the operational answer to 'why share?': communication-free sensing turns $m$ isolated, near-useless "
  "learners into one effective swarm. <i>(b) Positive scaling with team size.</i> Holding $n$, horizon "
  "$T$, and $\\rho$ fixed and growing the swarm $m{=}5\\to80$, RewardCF unseen skill RISES monotonically "
  "$0.08\\to0.43$ (non-overlapping CIs): more robots feed $\\sim mT\\rho$ broadcast observations into the "
  "SHARED structure, so each robot's skill on tasks it never serviced grows with the team (collective "
  "recovery, Theorem 11). The structure-free learner is FLAT ($m$ cannot help an agent that only learns "
  "its own row, Theorem 1). Positive scaling with team size, the swarm getting SMARTER as it grows, is "
  "unique to structure-sharing and the opposite of the usual interference penalty as agents are "
  "added.</p>")
