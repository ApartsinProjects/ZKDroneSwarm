...
A("<p><b>Recent results and honest scope.</b> <b>Precise scope (iff):</b> the CATEGORICAL "
  ...
  "= held-out informativeness is identifiable where in-sample is not. <b>The keystone, now bounded:</b> "
  "the per-drone recovery of $U$ from the persistent-masked broadcast, formerly invoked from centralized "
  "completion theory, is now pinned to an explicit per-drone condition. Drone $i$ recovers a target's "
  "factor $u_j$ (and so predicts the unseen pair) EXACTLY iff its own factor $p_i$ lies in the span of "
  "the factors of the visible teammates that engaged $j$ (an anchor block fixing the frame plus per-target "
  "spanning coverage); it is provably at the prior floor otherwise; under noise the error is bounded by "
  "the fold-in bound with the coverage degree controlling the noise term; and under non-adaptive "
  "exploration the condition self-achieves in $O((nd/\\rho m)\\log n)$ rounds, faster as the swarm grows. "
  "An oracle-reconstruction test on the actual coverage patterns confirms the threshold exactly (recovered "
  "pairs: error $0.000$; below the span condition: the prior floor). The one remaining open piece is the "
  "finite-time coverage rate under an adaptive exploiting policy (full statements, proofs, and the "
  "validation in <code>THEORY_FORMAL.md</code> and <code>P15_VALIDATION.md</code>).</p>")
...
A("<p><b>How to read the comparison.</b> The ZK-MRTA setting itself is new, so this is not 'our method "
  ...
  "reference ceilings. Provenance is explicit: our invented methods form ONE family, <b>SwarmCF</b> "
  "(the core RewardCF plus Bayesian, choice, de-confliction, unified and batch variants, shown as "
  "'SwarmCF-variant'), and PTF (SwarmCF-batch) is ours too, a strong batch-flavored hybrid we built from "
  "standard parts; MFSGD, ESTR, BPMF, SoftImpute, KNNCF and CLUB are named prior-work estimators we ADAPT "
  "to the setting (the algorithms are theirs, the decentralized broadcast-only application is part of our "
  "framework); the per-arm UCB and tabular learners are the structure-free paradigm we beat; and three "
  "full-information upper bounds (NOT competitors) bracket the top: the Oracle (knows the true reward), "
  "CentralClean (a centralized low-rank matcher that observes everything with NO noise and NO masking and "
  "assigns optimally, the strongest a centralized low-rank system can do without the true factors), and "
  "the noisy comms-full CTDE matcher. <b>Harness fact:</b> the "
