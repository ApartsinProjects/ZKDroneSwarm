...
if __name__ == "__main__":
    main()
        ...
        # learned_mf_matrix = extract_mf_self_view_matrix(policy)
        # if policy_type == "matrix_factorization_cf" and learned_mf_matrix is not None:
        ...
        #         score_matrix=learned_mf_matrix.tolist(),
        #         value_header="Score",
        #         value_matrix=learned_mf_matrix.tolist(),
        #         value_precision=3,
        #     )
...
# 
# 
#     oracle_reward_matrix = compute_initial_oracle_reward_matrix(
    #     drones_config=drones_config,
    #     targets_config=targets_config,
    #     class_attribute_mapping=config.mappings.class_attribute_mapping,
    #     weapon_damage_profile_mapping=config.mappings.weapon_damage_profile_mapping,
    ...
    #     class_attribute_mapping=config.mappings.class_attribute_mapping,
    #     weapon_damage_profile_mapping=config.mappings.weapon_damage_profile_mapping,
    #     score_matrix=oracle_reward_matrix.tolist(),
    # )
    # def compute_initial_oracle_reward_matrix(
#     drones_config: List[Dict[str, Any]],
#     targets_config: List[Dict[str, Any]],
...
#     reward_mode: str,
# ) -> np.ndarray:
#     """Compute the initial ground-truth reward matrix for the configured scenario."""
#     max_single_attribute_weapon_damage = max(
#         damage
#         for profile in weapon_damage_profile_mapping.values()
#         for damage in profile.values()
#     )

#     oracle_matrix = np.zeros((len(drones_config), len(targets_config)), dtype=np.float64)

#     for drone_idx, drone_cfg in enumerate(drones_config):
#         damage_profile = weapon_damage_profile_mapping[drone_cfg["weapon_type"]]
#         max_weapon_potential = float(sum(damage_profile.values()))
#         keys = sorted(damage_profile.keys())
#         weapon_vec = np.array([damage_profile[key] for key in keys], dtype=np.float64)
#         weapon_norm = float(np.linalg.norm(weapon_vec))

#         for target_idx, target_cfg in enumerate(targets_config):
#             hp_before_dict = class_attribute_mapping[target_cfg["class_type"]]
#             actual_damage = float(
#                 sum(min(hp_before_dict.get(attr_name, 0.0), damage) for attr_name, damage in damage_profile.items())
#             )

#             if reward_mode == "DAMAGE_EFFICIENCY":
#                 reward = actual_damage / max_weapon_potential if max_weapon_potential > 0 else 0.0
#             elif reward_mode == "DOMINANT_ATTRIBUTE":
#                 dominant_attr = max(hp_before_dict, key=hp_before_dict.get)
#                 reward = float(damage_profile.get(dominant_attr, 0.0)) / max_single_attribute_weapon_damage
#             elif reward_mode == "ATTRIBUTE_ALIGNMENT":
#                 damage_efficiency = actual_damage / max_weapon_potential if max_weapon_potential > 0 else 0.0
#                 target_vec = np.array([hp_before_dict.get(key, 0.0) for key in keys], dtype=np.float64)
#                 target_norm = float(np.linalg.norm(target_vec))
#                 cosine_sim = (
#                     float(np.dot(weapon_vec, target_vec)) / (weapon_norm * target_norm)
#                     if weapon_norm > 0.0 and target_norm > 0.0
#                     else 0.0
#                 )
#                 reward = damage_efficiency * cosine_sim
#             else:  # HP_REDUCTION
#                 hp_before = float(sum(hp_before_dict.values()))
#                 reward = actual_damage / hp_before if hp_before > 0 else 0.0

#             oracle_matrix[drone_idx, target_idx] = reward

#     return oracle_matrix


# def extract_mf_self_view_matrix(policy: IPolicy) -> Optional[np.ndarray]:
#     """Return one learned row per drone using each drone's own local MF model."""
#     if not isinstance(policy, MultiAgentPolicy):
#         return None

#     rows: List[np.ndarray] = []
#     for agent_id in sorted(policy.policies.keys()):
#         agent_policy = policy.policies[agent_id]
#         if not isinstance(agent_policy, MatrixFactorizationPolicy):
#             return None
#         rows.append(
#             np.array(
#                 [agent_policy.predict_reward(target_idx) for target_idx in range(agent_policy.num_targets)],
#                 dtype=np.float64,
#             )
#         )
