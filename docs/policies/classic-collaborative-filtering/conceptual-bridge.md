...
In the current project, however, the policy must respect the decentralized zero-knowledge setting. This means that a first practical baseline may use:

- a **private latent representation per drone**
- a **private target representation maintained locally by each drone**
- **no centralized coordination**
- **no shared hidden state across all agents**
...
- chosen actions
- observed rewards
- possibly limited observations of other agents
...
- it is **not yet the fullest version of collaborative learning**, because each drone primarily learns through its own local stream and private latent model

This clarification is valuable because it prevents conceptual confusion. It tells the reader that the policy is part of the recommendation-inspired family, while also acknowledging that it is adapted to satisfy the zero-knowledge, decentralized structure of the project.
...
4. clarify that the interaction matrix is **built from sparse experience**, not given מראש
