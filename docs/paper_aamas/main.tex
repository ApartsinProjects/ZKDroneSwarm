...
% Figures are the PNGs in ../figures/ (replace with vector PDFs for camera-ready).
...
fold-in. Per-agent $\Theta(d)$ vs tabular $\Theta(n)$: categorical on unseen pairs.
\end{theorem}
...
reaches near-oracle in $O(d)$ rounds.
\end{theorem}
...
\begin{figure}[t]\centering\includegraphics[width=0.7\linewidth]{../figures/F2_unseen_masking.png}
\caption{Unseen-pair skill vs observation density (categorical separation).}\label{fig:unseen}\end{figure}
\begin{figure}[t]\centering\includegraphics[width=0.7\linewidth]{../figures/F6_anytime.png}
\caption{Anytime cumulative reward: online CF earns from round one.}\label{fig:anytime}\end{figure}
\begin{figure}[t]\centering\includegraphics[width=0.8\linewidth]{../figures/F11_pareto.png}
...
We study the non-contention setting; contention turns the allocation into matching,
where coordination value may appear (a natural next step; the simulator's HP
depletion previews it). Under minimal assumptions, CF over the public broadcast lets
a swarm act on the unseen, onboard new tasks and agents, and earn from the first
round, with a categorical, theory-backed advantage and dominance over prior-art
low-rank methods in the limited-observability regime.
