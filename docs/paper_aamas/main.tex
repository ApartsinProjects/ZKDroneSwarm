...
since it is a property of the learned model. (ii) The \emph{operational} earned-reward
gap narrows as contention rises: CF leads clearly while there is slack but collapses
toward the matching floor under severe contention (pool $=15$, $2{:}1$), where collisions
not preferences bind, so without coordination the advantage is erased. (iii) A new
finding: the collective active-exploration bonus BACKFIRES under a shared pool (it
synchronizes probes, raising collisions), so plain $\epsilon$-greedy is more
contention-robust; de-synchronizing active exploration is the natural next step.
