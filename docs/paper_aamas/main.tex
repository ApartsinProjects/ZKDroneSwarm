...
Sensing is \emph{independent per observer}: each drone applies its own mask and draws its own
noise on every engagement, so the same action $a_k$ is recorded as $R_{k,a_k}+\eta_{ik}$ with
$\eta_{ik}\perp\eta_{i'k}$ for $i\neq i'$; the same engagement is read differently by different
drones, never a single shared value. This is passive sensing of public outcomes, not transmission; there is no message
...
one agent's probe lowers everyone's uncertainty, i.e.\ collective active learning). We
...
final-policy unseen). The further refinements form a \emph{scoped menu} rather than a
method zoo: a Bayesian variational-PMF estimator with calibrated confidence (EMCF), rank
self-determination (ARD), the choice channel with held-out informativeness, and contention
de-confliction are each an optional extension of the one core estimator, applied only when
its condition holds (noisy or coverage-scarce broadcast; unknown rank; extreme reward-noise
or unreliable teammates; capacity-1 contention) and otherwise reducing to the core.
...
\begin{figure}[t]\centering\includegraphics[width=0.7\linewidth]{../figures/pdf/F2_unseen_masking.pdf}
\caption{Unseen-pair skill vs observation density (categorical separation).}\label{fig:unseen}\end{figure}
\begin{figure}[t]\centering\includegraphics[width=0.7\linewidth]{../figures/pdf/F6_anytime.pdf}
\caption{Anytime cumulative reward: online CF earns from round one.}\label{fig:anytime}\end{figure}
\begin{figure}[t]\centering\includegraphics[width=0.8\linewidth]{../figures/pdf/F11_pareto.pdf}
...
\cite{jun2019bilinear,katariya2017rank1,kang2024lowrank,li2010contextual,chapelle2011empirical}
and online clustering of bandits \cite{gentile2014online} are centralized and/or
phase-structured; we are anytime and distributed. The closest neighbor is multi-user RL
with low-rank rewards \cite{nagaraj2022multiuser}, which shares the low-rank-across-agents
structure but CENTRALIZES trajectory aggregation, whereas we keep every agent's parameters
private and learn only from a partial, noisy broadcast. Recommender CF
\cite{koren2009matrix,hu2008collaborative,salakhutdinov2008bayesian} inspires our estimator
and the exposure-debiased choice channel. For contention, our communication-free
symmetry-breaking is in the spirit of no-communication multiplayer bandits, but applied on
top of a LEARNED low-rank preference rather than independent arms. We connect to the MRTA
taxonomy \cite{gerkey2004formal} and cooperative MARL \cite{amato2024ctde}, but remove
communication, priors, and shared state (the CTDE workhorses centralize training; the
admissible no-communication case reduces to our per-agent bandit baselines). Our
contribution is COMPOSITIONAL: this exact combination (decentralized, broadcast-only,
online, sample-starved, with an unseen-pair categorical separation) is the open niche, while
...
the matching floor only at severe contention. (iii) We WIN even there with a decentralized,
communication-free fix. Keeping the CF estimate but replacing greedy argmax with a FIXED
private per-target offset (deterministic symmetry breaking) earns $0.105\,[0.096,0.114]$ at
pool $=15$ versus $\approx0.05$ for argmax-CF/PTF (non-overlapping, $\sim2\times$). Making
that offset \emph{self-tuning} (ContentionAdaCF: scale it by each drone's own observed loss
rate, gate it by a zero-knowledge scarcity signal of offer size vs swarm size, and fall back
to $\epsilon$-greedy when targets are plentiful) yields a SINGLE policy that is
best-or-statistically-tied at \emph{every} contention level: it matches plain CF when targets
are abundant (pool $=240$: $0.448$ vs $0.439$, with the lowest collision rate) and wins by
$\sim2\times$ at severe contention (pool $=15$: $0.100$ vs $\le0.06$ for every no-offset
method), with no communication. (iv) De-confliction must use \emph{private, fixed} randomness: per-round
softmax and shared-signal (popularity / collective count-bonus) routing both backfire by
re-colliding or re-synchronizing.
(v) Against the field's two recognized no-comms de-confliction primitives, each given our SAME CF
utility so the comparison isolates the primitive, a CBBA-style auction with public-loss backoff and a
SIC-MMAB / musical-chairs randomized re-seating, the fixed private offset WINS at severe contention
($0.100$-$0.105$ vs CBBA-lite $0.064$ vs re-seating $0.028$ at pool $=15$, non-overlapping CIs);
CBBA-lite is competitive only when targets are plentiful (it is a credible baseline, not a strawman),
and randomized re-seating is worst (it injects collisions).
(vi) \textbf{Capstone:} uniting the confidence core, the loss-self-gated offset, and a loss-and-abundance
gated exploration anneal into ONE policy (UnifiedCF) is best-or-statistically-tied across the standard,
churn, and BOTH contention extremes on the same seeds, with no per-regime tuning, so the design space
collapses to a single self-tuning policy.
...
\textbf{Scope: non-stationarity (churn).} Under continuous target turnover (departures plus
fresh arrivals), PLAIN exploitative CF does not suffice: fold-in's order-$d$ probe latency is
outpaced by churn on the freshest arrivals, where it even trails a structure-free optimist
($0.074$ vs $0.132$). The fix is the same confidence-directed exploration from our refinements
menu: a CF estimator that probes uncertain (fresh) targets and folds them in (ActiveCF
count-bonus, or EMCF predictive-variance UCB) dominates under churn on both the active set and
the freshest arrivals (EMCF active $0.842$ vs $0.619$ structure-free; fresh $0.371$ vs $0.132$,
non-overlapping CIs). The swarm stays adapted under continuous change, provided it unites
low-rank fold-in with directed newcomer-probing.
