...
transfers to a realistic simulator.
\end{abstract}
...
realistic simulator. An interactive tutorial with all figures and proofs is
...
realistic tabula\_drone simulator (skill $\approx 0.81$, beating the env's SGD-MF and
...
\textbf{Scope: contention.} Our formal results assume non-contention (a target can be
engaged by more than one drone). Hard contention turns allocation into matching, where
explicit coordination may add value beyond preference quality. We expect the
\emph{categorical} unseen-pair and onboarding results to persist, because they concern
the quality of each agent's preference estimate (which contention does not change),
while the operational gap may narrow as collisions, not preferences, become the
bottleneck. Preliminary evidence: the realistic tabula\_drone validation already runs
\emph{with} HP depletion, a soft contention in which engaged targets are drawn down, and
our method still leads (skill $\approx 0.81$, beating SGD-MF and UCB-Indep). A
controlled hard-contention study (capacity-limited targets, with and without a
broadcast-only de-confliction rule) is the natural next step.
