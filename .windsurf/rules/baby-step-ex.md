...
Produce:
- Atomic Change - break work into the smallest possible unit that can be compiled, tested, or run.
- Validation Outcome - each step must demonstrate a clear result (e.g., code compiles, test passes, CLI runs).
...
Produce:
- Documentation - notes, logs, or decision records for each step.
- Traceability - make sure the path of changes is clear and reviewable.
...
Produce:
- Single Outcome - complete one defined accomplishment before starting another.
...
Produce:
- Finished Step - implemented, validated, and documented before moving on.

Guardrails:
- A step is not done until it meets all criteria above.